...
class CrawlCommand extends Command
{
     ...
     */
    protected $description = '爬取拉勾广州所有PHP职位';
    ...
    public function handle()
    {
        ...
        $positionData = $this->runCurlCommand();
    ...
    protected function runCurlCommand(int $page = 1): \stdClass
    {
        // 更换 cookie 不再影响缓存
        $key = 'lagou:ajax:position:' . md5($this->buildCurlCommand('', $page));

        return Cache::remember($key, now()->addHour(), function () use ($page) {
            $cookie  = $this->option('cookie');
            $command = $this->buildCurlCommand($cookie, $page);
